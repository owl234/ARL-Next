...
UPDATER_SCRIPT="$UPDATER_DIR/updater_test.py"
SERVICE_FILE="/etc/systemd/system/arl-updater-test.service"
...
EOF
        ...
        if [ "$ARL_UPDATER_SKIP_RESTART" != "1" ]; then
            systemctl enable arl-updater-test.service >/dev/null 2>&1
            systemctl restart arl-updater-test.service
            echo "✅ 系统更新服务启动成功！"
        else
...
while [ $WAIT_TIME -lt $MAX_WAIT ]; do
    if docker exec arl-web-test curl -s http://127.0.0.1:5000/ > /dev/null 2>&1; then
...
FAILED_SERVICES=$(docker compose -f docker-compose.test.yml ps --status exited --status restarting --services)

if [ -n "$FAILED_SERVICES" ]; then
    echo "❌ 警告：部分服务启动失败或正在无限重启中！"
    echo "异常服务列表："
    echo "$FAILED_SERVICES"
    echo "👉 建议稍后通过终端进入服务器执行 'docker compose -f docker-compose.test.yml logs <服务名>' 查看具体报错。"
else
...
echo "🎉 部署完成！"
echo "🌟 所有组件均已在 Docker 私有网络内隔离启动，公网仅对外暴露前端 5174 端口。"
echo "👉 请通过浏览器访问以下地址之一登录系统："
echo "   - 本地/局域网访问: https://$LOCAL_IP:5174"
if [ -n "$PUBLIC_IP" ] && [ "$PUBLIC_IP" != "$LOCAL_IP" ]; then
    echo "   - 公网访问:        https://$PUBLIC_IP:5174"
fi

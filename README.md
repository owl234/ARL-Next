...
* **高并发异步调度**
  以 **Redis** 为消息中枢并开启 AOF 持久化，配合 Celery 分布式工作节点为执行引擎。支持将核心扫描 Worker 和专门的 GitHub 敏感信息监控 Worker 分散部署，并可在系统设置页面实时调整 Worker 并发数。
* **现代 Web 无头渲染**
...
# 3. 启动所有服务容器（镜像直接从 GHCR 拉取，无需本地 build）
